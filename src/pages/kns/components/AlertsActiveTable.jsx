...
// project import
import { getKNSAlertsActive } from '@pages/kns/redux/knsAlertsActiveSlice';
...
const AlertsTableActive = ({ knsID, status }) => {
    const dispatch = useDispatch();
    const { data, loading, error } = useSelector((state) => state.knsAlertsActive);
    const { reqData, reqLoading, reqError } = useSelector((state) => state.knsAlertDisable);
    ...
    useEffect(() => {
        dispatch(getKNSAlertsActive({ id: knsID, page: page, perPage: rowsPerPage }));
        setFirstLoad(true);
    ...
    return (
        <Box>
            <ModalDissble props={{status:status , tableAlertPrority: tableAlertPrority, tableDate: tableDate, tableAlertName:tableAlertName, tableAlertID: tableAlertID, open: open, tableAlertState: tableAlertState}} disableSingleAlert={disableSingleAlert} handleClose={handleClose} />
            <TableContainer
                sx={{
                    width: '100%',
                    overflowX: 'auto',
