...
const Status = ({ title, text, status, extraText, newDesign = false}) => {
    ...
    return (<>
        { newDesign ? <Tooltip title={`${text} - ${extraText}`} placement="top"><Alert severity={status === true ? "success" : "error"} sx={{ mt: 0, mb: -1.4 }}>
        {title}
    </Alert></Tooltip>
    :
        <MainCard contentSX={{ p: 2.25 }}>
                <Grid container rowSpacing={4.5} columnSpacing={2.75}>
                    ...
                    <Grid item xs={4} sm={4} md={4} lg={4} display="flex" justifyContent="flex-end">
                        <Avatar alt="profile user" src={status === true ? isSuccess : isError} sx={{ width: 50, height: 50, mr: 1, mt: 1.22 }} />
                    </Grid>
                </Grid>
        </MainCard>}
        </>)
